...
BEGIN
    ...
    FOR v_record IN 
        ...
        WHERE 
            u.face_id IS NULL
    LOOP
        -- Update users table with face_id
        UPDATE public.users
        SET 
            face_id = v_record.face_id,
            face_updated_at = NOW()
...
BEGIN
    ...
    LOOP
        ...
        -- Try to get face_id from different sources
        ...
        IF v_face_id IS NULL THEN
            SELECT face_id INTO v_face_id FROM public.user_faces WHERE user_id = user_id AND face_id IS NOT NULL;
        END IF;
        
        -- If we found a face_id, update all tables
        IF v_face_id IS NOT NULL THEN
            -- Update users table
            UPDATE public.users
            SET face_id = v_face_id
            WHERE id = user_id AND (face_id IS NULL OR face_id <> v_face_id);
            ...
            WHERE public.face_data.face_id IS NULL OR public.face_data.face_id <> v_face_id;
            
            -- Update or insert into user_faces

 ...
 */
const LargeDataTable = ({ data = [], fetchData = null, renderItem = null, initialLimit = 50, itemProcessor = item => item, className = '' }) => {
    ...
    const defaultRenderItem = (item, index) => (_jsxs("div", { className: "p-4 border-b border-gray-200 hover:bg-gray-50", children: [_jsxs("h3", { className: "font-medium", children: ["Item ", index + 1] }), _jsx("pre", { className: "text-sm text-gray-600 mt-1 overflow-hidden text-ellipsis", children: JSON.stringify(item, null, 2) })] }));
    ...
    return (_jsxs("div", { className: `large-data-container ${className}`, children: [_jsxs("div", { className: "mb-4 text-sm text-gray-500", children: ["Displaying ", displayData.length, " of ", data.length, " items"] }), _jsx("div", { className: "virtualized-container h-[600px] border border-gray-200 rounded-lg", children: _jsx(VirtualizedList, { items: displayData, renderItem: renderItem || defaultRenderItem, itemHeight: 100, overscan: 10, infiniteScroll: !!fetchData, loadMore: loadMore, hasMore: hasMore, isLoading: isLoading, loadingMessage: "Loading more data...", batchSize: 50 }) })] }));
};

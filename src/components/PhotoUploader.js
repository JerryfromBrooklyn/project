...
export const PhotoUploader = ({ eventId, onUploadComplete, onError }) => {
    ...
    const processUploads = async () => {
        console.log("🔵 [PhotoUploader] processUploads triggered."); // Log start
        if (!validateMetadata()) {
            console.error("🔴 [PhotoUploader] Metadata validation failed."); // Log validation failure
            return;
        }
        console.log("🟢 [PhotoUploader] Metadata validation passed."); // Log validation success
        ...
        console.log(`🔵 [PhotoUploader] Prepared ${newUploads.length} items for upload queue.`); // Log queue prep

        // Process uploads
        for (const upload of newUploads) {
            try {
                console.log(`🔵 [PhotoUploader] Starting processing for upload ID: ${upload.id}, File: ${upload.file.name}`); // Log start of loop item
                ...
                console.log(`🔵 [PhotoUploader] Calling awsPhotoService.uploadPhoto for ID: ${upload.id}`); // Log before service call
                // Upload photo using AWS S3 service
                const result = await awsPhotoService.uploadPhoto(
                    upload.file, 
                    eventId,
                    upload.folderPath,
                    { ...photoMetadata, uploadedBy: user?.id },
                    handleProgress
                );
                if (result.success) {
                    const updatedUpload = {

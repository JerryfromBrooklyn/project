 ...
 */
const VirtualizedList = ({ items = [], renderItem, itemHeight = 50, overscan = 5, infiniteScroll = false, loadMore = () => { }, hasMore = false, loadingMessage = 'Loading more items...', batchSize = 50, isLoading = false, className = '', }) => {
    ...
    const updateVisibleRange = useCallback(() => {
        ...
        const endIndex = Math.min(totalItems - 1, Math.floor((scrollTop + clientHeight) / itemHeight) + overscan);
    ...
    return (_jsx("div", { ref: containerRef, className: `virtualized-list-container overflow-auto ${className}`, style: { position: 'relative', height: '100%' }, children: _jsxs("div", { style: { height: `${totalHeight}px`, position: 'relative' }, children: [visibleItems.map((item, index) => {
                    ...
                    return (_jsx("div", { style: {
                        ...
                        }, children: renderItem(item, actualIndex, true) }, `virtual-item-${actualIndex}`));
                }), infiniteScroll && isLoading && (_jsx("div", { style: {
                        padding: '20px',
                        textAlign: 'center',
                        position: 'absolute',
                        bottom: 0,

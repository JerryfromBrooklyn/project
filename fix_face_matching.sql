-- Check and fix face registration flow

-- 1. Make sure face_data table has correct structure
ALTER TABLE face_data 
  ADD COLUMN IF NOT EXISTS is_verified BOOLEAN DEFAULT false;

-- 2. Fix the face matching process by updating views and functions

-- Fix the function that handles face matches
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.confidence != NEW.confidence) THEN
    ...
    UPDATE photos 
    SET 
      face_ids = array_append(face_ids, NEW.face_id),
...
-- Create a function to close the loop between face_data and photo uploads
...
BEGIN
  ...
  LOOP
    ...
    IF v_photo_record.faces @> jsonb_build_array(
      jsonb_build_object('faceId', v_face_id)
    ) THEN
...
-- Create a function that processes uploads with face data
...
-- Make sure photo_faces table has the right structure
ALTER TABLE photo_faces
  ADD COLUMN IF NOT EXISTS face_id TEXT;

-- Create a better view for user matched photos
...
-- Create a function to manually trigger a face match scan
  ...
  LOOP
    ...
    FOR v_photo_record IN 
      ...
      WHERE 
        ...
        jsonb_array_length(p.faces) > 0
    LOOP
      -- Check if any face in the photo matches this face_id
      IF EXISTS (
        SELECT 1
        FROM jsonb_array_elements(v_photo_record.faces) AS face
